...
jQuery.GlizyRegisterType('PdfEditor', {
    ...
    __construct: function () {
        ...
        self.dragres = false;
        self.lastMediaUrlId = false;
        self.init = function() {
            self.baseUrl = el.data('base-url');
            ...
            if (!value) {
                // correzione del bug che cancellava il numero di pagine
                // per risolverlo legge il titolo della pagina
                value = $('#__title').val().replace(/Pagina /i, '');
            }
            if (value) {
                if (value==parseInt(value)) {
                    self.data = {pagNum: value, hotspots: []};
                    self.storeValue();
                } else {
                    self.data = JSON.parse(value);
                }
                self.setImage(self.data.pagNum);
                self.loadHotspots();
            }
        };

        self.setImage = function(pageNum) {
            if (pageNum) {
                self.image.attr('src', self.baseUrl+'page_'+pageNum+'_1.5.png');
                self.image.show();
            }
        };
        ...
        {
            if (self.lastMediaUrlId!==null) {
                self.hotspot.srcMedia = media;
                $('#urlMedia_'+self.lastMediaUrlId).val(media.title);
            }
        }
        ...
        self.addTrack = function(id) {
            ...
            var urlMediaDiv = $('<div/>')
                .attr('id', 'urlMediaDiv_'+id)
                .addClass('list-container')
                .css('clear', 'both');
            var label = $('<label/>')
                .attr('for', 'urlMedia_'+id)
                .addClass('label')
                .addClass('list-label')
                .text('Media');
            urlMediaDiv.append(label);
            var urlMedia = $('<input/>')
                .attr('id', 'urlMedia_'+id)
                .attr('type', 'text')
                .attr('readonly', true)
                .addClass('list-url');
                urlMediaDiv.click(function() {
                     var url = self.glizyOpt.mediaPicker+'&mediaType=ALL';
                    ...
                    self.lastMediaUrlId = id;
                    Glizy.lastMediaPicker = self;
                })
            urlMediaDiv.append(urlMedia);
            hotspotContainer.append(urlMediaDiv);
            urlMedia.val(self.hotspot.srcMedia ? self.hotspot.srcMedia.title : '');
        ...
        self.changeType = function($type, id) {
            switch($type.val()) {
                case 'linkEx':
                    $('#urlIntDiv_'+id).hide();
                    $('#urlMediaDiv_'+id).hide();
                    $('#urlDiv_'+id).show();
                    self.hotspot.srcInt = '';
                    $('#urlInt_'+id).val('');
                    $('#urlMedia_'+id).val('Seleziona media');
                    self.hotspot.srcMedia = '';
                    break;
                case 'link':
                    $('#urlDiv_'+id).hide();
                    $('#urlMediaDiv_'+id).hide();
    ...
    openDialogCallback: function() {
        ...
        $frame.load(function () {
            jQuery( "img.js-glizyMediaPicker", $frame.contents().get(0)).click( function(){
                var $img = jQuery( this );
                Glizy.lastMediaPicker.setMedia({
                        id: $img.data( "id" ),
