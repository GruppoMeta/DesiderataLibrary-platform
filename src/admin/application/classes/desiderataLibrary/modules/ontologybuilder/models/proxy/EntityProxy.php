<?php
class desiderataLibrary_modules_ontologybuilder_models_proxy_EntityProxy extends GlizyObject
{
    public function loadContent($entityId, $status='PUBLISHED')
    {
        ...
        $fieldTypeService = $application->retrieveProxy('desiderataLibrary.modules.ontologybuilder.service.FieldTypeService');
    ...
    public function saveContent($data, $publish=true)
    {
        ...
        $document->addField(new org_glizy_dataAccessDoctrine_DbField('subtitle', Doctrine\DBAL\Types\Type::STRING, 255, false, null,'', false));
        $document->subtitle = property_exists($data, 'subtitle') ? $data->subtitle : '';
        ...
        $document->url = property_exists($data, 'url') ? $data->url : '';

        $document->addField(new org_glizy_dataAccessDoctrine_DbField('profile', Doctrine\DBAL\Types\Type::TARRAY, 255, false, null, '', false));
        $document->profile = property_exists($data, 'profile') ? $data->profile : '';
        ...
        try {
            ...
            if ($speakingUrlProxy) {
                ...
                if ($originalUrl != $document->url) {
                    //valida l'url
                    if (!$speakingUrlProxy->validate($document->url, $languageId, $id, 'desiderataLibrary.modules.ontologybuilder.content')) {
                        ...
                        $speakingUrlProxy->addUrl($document->url, $languageId, $id, 'desiderataLibrary.modules.ontologybuilder.content', $options);
    ...
    public function loadContentFrontend($entityId)
    {
		...
		$localeService = $application->retrieveProxy('desiderataLibrary.modules.ontologybuilder.service.LocaleService');
