<?php
class desiderataLibrary_controllers_siteTree_ajax_GetSiteTree extends org_glizycms_contents_controllers_siteTree_ajax_GetSiteTree
{
    private $pageTypes;
    ...
    private function addNode($menu, $isRoot=false) {
        ...
        $data = $contentProxy->readContentFromMenu($menu->menu_id, gruppometa_easybook_Easybook::getLanguage());
        if (property_exists($data, 'number')) {
            $title = $data->number.' '.$title;
        }

        $icon = 'page';
        if ( $menu->menu_pageType=='Empty' || $menu->menu_pageType=='EmptyPdf') {
            $icon = 'folder';
        } else if ( $menu->menu_pageType=='Alias') {
            $icon = 'alias';
        }

        if ( $menu->menu_type == 'HOMEPAGE' || $menu->menu_pageType == 'Publication' || $menu->menu_pageType == 'PublicationPdf') {
            $icon = 'home';
