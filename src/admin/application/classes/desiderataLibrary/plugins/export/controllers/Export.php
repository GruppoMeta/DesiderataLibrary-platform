<?php
class desiderataLibrary_plugins_export_controllers_Export extends org_glizy_mvc_core_Command
{
    public function execute($menuId)
    {
        ...
        if ($menuId && is_array($menuId)) {
            ...
            foreach($menuId as $v) {
                ...
                $jobFactory->createJob('desiderataLibrary.plugins.export.services.ExportService',
                    $params,
                    'Esportazione pubblicazione: '.$menu->menudetail_title.' id: '.$v,
                    'BACKGROUND'
                );
            }
